...
type (
	...
	Secret struct {
		...
		AlifToken           string `env-required:"true" yaml:"log_level"   env:"ALIF_TOKEN"`
	...
	Payment struct {
		...
		AlifBaseUrl        string `env-required:"true" yaml:"alif_base_url" env:"ALIF_BASE_URL"`
	}
	Integration struct {
...
func Load() Config {
	...
	c.Secret.AlifToken = cast.ToString(GetOrReturnDefaultValue("ALIF_TOKEN", "6fb5619d-8c30-4e85-a1e3-3f4d142498a0"))
	...
	c.AlifBaseUrl = cast.ToString(GetOrReturnDefaultValue("ALIF_BASE_URL", "https://api-dev.alifpay.uz/v2"))
