...
func Load() Config {
	...
	c.Integration.ClickApiUrl = cast.ToString(GetOrReturnDefaultValue("CLICK_ENDPOINT_URL", "http://localhost:8080"))
	c.Integration.PaymeApiUrl = cast.ToString(GetOrReturnDefaultValue("PAYME_GO_ENDPOINT_URL", "http://localhost:8080"))
	c.Integration.UzumApiUrl = cast.ToString(GetOrReturnDefaultValue("UZUM_ENDPOINT_URL", "http://localhost:8080"))
	c.Integration.AlifApiUrl = cast.ToString(GetOrReturnDefaultValue("ALIF_BASE_URL", "https://api-dev.alifpay.uz/v2"))
	c.Integration.AlifToken = cast.ToString(GetOrReturnDefaultValue("ALIF_TOKEN", "6fb5619d-8c30-4e85-a1e3-3f4d142498a0"))
	c.Integration.OnecApiUrl = cast.ToString(GetOrReturnDefaultValue("BASE_URL_1C", "http://localhost:8080"))
	c.Integration.OnecApiUsername = cast.ToString(GetOrReturnDefaultValue("BASE_USERNAME_1C", "pharma"))
	c.Integration.OnecApiPassword = cast.ToString(GetOrReturnDefaultValue("BASE_PASSWORD_1C", "password"))
	c.Integration.TasnifApiUrl = cast.ToString(GetOrReturnDefaultValue("SOLIQ_IKPU_BASE_URL", "http://localhost:8080"))
	c.Integration.NoorApiUrl = cast.ToString(GetOrReturnDefaultValue("NOOR_BASE_URL", "http://localhost:80"))
	c.Integration.NoorApiToken = cast.ToString(GetOrReturnDefaultValue("NOOR_TOKEN", "token"))
	c.Integration.DmedApiUrl = cast.ToString(GetOrReturnDefaultValue("DMED_BASE_URL", "http://localhost:80"))
	c.Integration.DmedApiToken = cast.ToString(GetOrReturnDefaultValue("DMED_TOKEN", "token"))
