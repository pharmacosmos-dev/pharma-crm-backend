...
func (s *Services) ReturnDetailList(param *domain.ReturnDetailParam) ([]domain.ReturnDetail, int64, error) {
	...
	query := s.db.
		...
		Select(`
			...
			FLOOR(transfer_details.rejection_count)::integer AS rejection_pack,
			ROUND((transfer_details.rejection_count - FLOOR(transfer_details.rejection_count)) * p.unit_per_pack)::integer AS rejection_unit,
