...
func (s *Services) DashboardStockImportStatistic(ctx context.Context, params *domain.DashboardQueryParam) (*domain.DashboardStockStatistic, error) {
	...
	query := `
		...
		SELECT
			COALESCE(SUM((COALESCE(idet.retail_price_vat, td.retail_price, sp.retail_price) / NULLIF(p.unit_per_pack, 0)) * sp.unit_quantity), 0) AS total_import_amount,
			...
			COALESCE(SUM(
				CASE WHEN idet.id IS NOT NULL OR td.id IS NOT NULL THEN
					((sp.retail_price - COALESCE(idet.retail_price_vat, td.retail_price, 0)) - COALESCE(ra.price_diff_sum, 0)) / NULLIF(p.unit_per_pack, 0) * sp.unit_quantity
				ELSE 0 END
			), 0) AS untracked_price_change_amount
...
func (s *Services) DashboardUntrackedPriceChanges(ctx context.Context, params *domain.DashboardQueryParam) ([]domain.UntrackedPriceChangeItem, error) {
	...
	query += `
		...
		SELECT * FROM diffs
		ORDER BY ABS(untracked_amount) DESC
		LIMIT ? OFFSET ?`
	args = append(args, params.Limit, params.Offset)
