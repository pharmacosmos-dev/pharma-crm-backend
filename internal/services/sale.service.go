...
func (s *Services) CreateOnlineSale(ctx context.Context, req *domain.OnlineSaleCreate) (*domain.Sale, error) {
	...
	VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING *`,
		...
		req.CustomerId,
		s.generateDisplayId(),
		constants.PaymentTypeUzumTezkor,
		true,
		req.ClientComment,
		req.VendorOrderId,
	).Scan(&sale).Error
		...
		if sale.Stage < constants.SaleStagePayFinished {
			...
			if req.LoyaltyCardBarcode != "" {
				if req.LoyaltyCard > 0 {
					updates["cash_back"] = (req.Cash + req.Humo + req.Uzcard + req.Click + req.Payme + req.Alif + req.Uzum + req.UzumTezKor) * (float64(sale.Customer.LoyaltyCardPercent) / 100)
				} else {
...
func (s *Services) EposResult(ctx context.Context, req *domain.EposResponseRequest, user *domain.EmployeeClaims) (*domain.Sale, error) {
	// Normalize response_data and update req.Error/Status if needed
...
func (s *Services) AddSaleBonuses(sale *domain.Sale, req []domain.CartItemWithProduct, loyaltyCardBarcode string) {
	...
	if sale.CashBack > 0 || loyaltyCardBarcode != "" {
		err := s.db.Exec(`
		UPDATE customers SET balance = balance + ? WHERE id = ? and is_active = true`, sale.CashBack, sale.CustomerId).Error
		if err != nil {
	...
	if sale.LoyaltyCard > 0 && sale.CustomerId != "" {
		err := s.db.Exec(`
		UPDATE customers SET balance = balance - ?, spending_from_balance = spending_from_balance + ? WHERE id = ? AND is_active = true`,
