...
vozvrat_pending_data AS (
    SELECT
        tr.id, tr.public_id::int, 5 AS entry_type, tr.created_at,
        s.name AS store_name,
        SUM(td.scanned_count) * vd.unit_per_pack * (-1) AS quantity,
        SUM(td.scanned_count * td.retail_price) * (-1) AS sum,
        tr.name as name,
        tr.status,
        NULL::jsonb AS metadata,
        vd.unit_per_pack
...
transfer_in_pending_data AS (
    SELECT
        ...
        tr.created_at,
        fs.name || ' -> ' || ts.name as store_name,
        SUM(td.scanned_count) * vd.unit_per_pack AS quantity,
        SUM(td.scanned_count * td.retail_price) AS sum,
        tr.name as name,
        tr.status,
        NULL::jsonb AS metadata,
        vd.unit_per_pack
