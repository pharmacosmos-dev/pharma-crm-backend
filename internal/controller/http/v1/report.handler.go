...
func (h *ReportHandler) GetProductsReportExcel(c *gin.Context) {
	...
	// Headerlar
	headers := []string{"ID", "Филиал", "Наименование", "Производитель", "Серия", "Срок Годности", "Кол-во", "Цена прихода", "Цена продажная", "Сумма прихода", "Сумма продажная", "Сумма наценки", "Сумма НДС", "Общее Сумма скидка", "Накопительная", "Дата продажи", "Время продажи", "Пользователь", "ID ЧЕКА", "МК кол-во"}
	...
	for i, value := range res {
		...
		f.SetCellValue(sheetName, "O"+row, value.LoyaltyCardAmount)
		f.SetCellValue(sheetName, "P"+row, value.CompletedAt.Add(time.Hour*5).Format(time.DateOnly))
		f.SetCellValue(sheetName, "Q"+row, value.CompletedAt.Add(time.Hour*5).Format(time.TimeOnly))
		f.SetCellValue(sheetName, "R"+row, value.FullName)
		f.SetCellValue(sheetName, "S"+row, helper.SaleTypeToRussian(value.SaleType, value.SaleNumber))
		f.SetCellValue(sheetName, "T"+row, value.MarkingCount)
	}
...
func (h *ReportHandler) StoreReportAmountExport(c *gin.Context) {
	...
	for i, value := range res {
		...
		f.SetCellValue(sheetName, "J"+row, value.Uzum)
		f.SetCellValue(sheetName, "K"+row, value.UzumTezKor)
		f.SetCellValue(sheetName, "L"+row, value.LoyaltyCardAmount)
		f.SetCellValue(sheetName, "M"+row, value.ReturnAmount)
		f.SetCellValue(sheetName, "N"+row, value.TotalAmount)
		f.SetCellValue(sheetName, "O"+row, value.ChequeCount)
	}
