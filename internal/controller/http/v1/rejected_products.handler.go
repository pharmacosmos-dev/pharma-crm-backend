...
// @Accept json
// @Produce json
// @Param body body domain.CreateReservedDocumentRequest true "Create reserved document request"
// @Success 200 {object} v1.Response{data=domain.Reserved}
// @Failure 400 {object} v1.Response
// @Failure 500 {object} v1.Response
// @Router /rejected-products/reserved-document [post]
func (h *RejectedProductsHandler) CreateReservedDocument(c *gin.Context) {
	var body domain.CreateReservedDocumentRequest
	...
	if user.StoreId == "" {
		handleResponse(c, BadRequest, "store_id not found in token")
		return
	}
	...
	reserved, err := h.service.CreateReservedDocument(ctx, user.StoreId, &body, user.UserId)
	if err != nil {
		handleServiceResponse(c, InternalError, err)
		return
	}

	handleResponse(c, OK, reserved)
}

// godoc AddReservedDetails
// @Summary Add or update reserved details
// @Description Add product to reserved document or update quantity if already exists
// @Tags rejected-products
// @Security     BearerAuth
// @Accept json
// @Produce json
// @Param reservedId path string true "Reserved Document ID"
// @Param body body domain.AddReservedDetailsRequest true "Add reserved details request"
// @Success 200 {object} v1.Response{data=domain.ReservedDetails}
// @Failure 400 {object} v1.Response
// @Failure 500 {object} v1.Response
// @Router /rejected-products/reserved-details/{reservedId} [post]
func (h *RejectedProductsHandler) AddReservedDetails(c *gin.Context) {
	reservedId := c.Param("reservedId")
	if reservedId == "" {
		handleResponse(c, BadRequest, "reservedId is required")
		return
	}

	var body domain.AddReservedDetailsRequest
	...
	detail, err := h.service.AddReservedDetails(ctx, reservedId, &body, user.UserId)
	if err != nil {
		handleServiceResponse(c, InternalError, err)
		return
	}
...
// @Accept json
// @Produce json
// @Param body body domain.InsertReservedDetailsDirectRequest true "Insert reserved details direct request"
// @Success 200 {object} v1.Response{data=domain.ReservedDetailsWithProduct}
// @Failure 400 {object} v1.Response
// @Failure 500 {object} v1.Response
// @Router /rejected-products/reserved-details [post]
func (h *RejectedProductsHandler) InsertReservedDetailsDirect(c *gin.Context) {
	var body domain.InsertReservedDetailsDirectRequest
	...
	detail, err := h.service.InsertReservedDetailsDirect(ctx, &body, user.UserId)
	if err != nil {
		handleServiceResponse(c, InternalError, err)
		return
	}
